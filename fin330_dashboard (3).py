...
def apply_theme():
    ...
    <style>
        ...
        /* ── FIX: Sidebar collapse/expand button — always show arrows, never text ── */
        /* Target the collapsed control button (the re-open arrow when sidebar is hidden) */
        ...
        /* Hide any raw text nodes / ligature strings inside the toggle buttons */
        button[data-testid="baseButton-headerNoPadding"] > div > p,
        [data-testid="collapsedControl"] > div > p {{
            display: none !important;
        }}
        /* Ensure Material Symbols icon renders as glyph on BOTH open and collapsed buttons */
    ...
    <script>
    // ── AUTO-COLLAPSE SIDEBAR ON "RUN ANALYSIS" CLICK ──
    // Watches for clicks on the Run Analysis button and collapses the sidebar.
    (function() {{
        function collapseSidebar() {{
            // Streamlit's collapse button sits inside [data-testid="stSidebar"]
            // as a button with aria-expanded="true"
            var btn = document.querySelector('button[data-testid="baseButton-headerNoPadding"]');
            if (btn) {{
                btn.click();
            }}
        }}
        ...
        function attachRunListener() {{
            // The Run Analysis button text is uppercase "RUN ANALYSIS"
            var buttons = document.querySelectorAll('.stButton > button');
            buttons.forEach(function(btn) {{
                var txt = btn.innerText.trim().toUpperCase();
                if (txt === 'RUN ANALYSIS' && !btn.dataset.collapseAttached) {{
                    btn.dataset.collapseAttached = 'true';
                    btn.addEventListener('click', function() {{
                        // Small delay so Streamlit registers the click first
        ...
        // Re-run attachment on every DOM mutation because Streamlit re-renders
        var observer = new MutationObserver(attachRunListener);
        observer.observe(document.body, {{ childList: true, subtree: true }});
        // Initial attach attempt
        setTimeout(attachRunListener, 500);
    }})();
    </script>
